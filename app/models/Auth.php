...
class Auth extends Model
{
    ...
    {
        if (strlen(intval($username)) == strlen($username)) {
     ...
     * @return array
     */
    public function getGames($user_id = 0)
    {
        $sql = "SELECT g.game_id FROM `games` g, `user_game` r WHERE r.user_id=:user_id AND g.id=r.game_id";
        $bind = array('user_id' => $user_id);
        $query = DI::getDefault()->get('dbBackend')->query($sql, $bind);
        $query->setFetchMode(Db::FETCH_ASSOC);
        $data = $query->fetchAll();
        ...
        return array_column($data, 'game_id');
    }
    ...
    {
        $sql = "SELECT
    ch.id, ch.channel, ch.channel_name
FROM
    `user_channel` uc,
    `channels` ch
WHERE
    uc.channel_id = ch.id AND uc.user_id = {$user_id}";
