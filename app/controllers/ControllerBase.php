...
class ControllerBase extends Controller
{
    ...
    public function initialize()
    {
        ...
        if ($_POST) {
            $logModel = new Log();
            $logModel->operationLog();
        }


        // check auth
        if ($this->config->setting->security_plugin) {
            if (!$this->_user_id || !$this->session->get('is_login') || !$this->session->get('resources')) {
    ...
    public function afterExecuteRoute(Dispatcher $dispatcher)
    {
        if (!isset($_SERVER['HTTP_X_PJAX'])) {
            //项目切换
            if (is_null(self::$web_site)){
                self::$web_site = App::find(['columns'=>'name,domain'])->toArray();
            }
